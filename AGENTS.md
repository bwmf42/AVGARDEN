# AV/GARDEN Agent Guide

本文件记录 AV/GARDEN 项目专属规则，供 AI agent 或维护者快速理解、配置和验证项目。

## 项目定位

AV/GARDEN 是面向 NAS 和家庭服务器的 AV 媒体下载、整理和浏览系统：

- 用户输入番号或从每日推荐中选择条目。
- Worker 优先使用 Sukebei/weekly 磁链并交给 qBittorrent。
- 只有未找到磁链时，才 fallback 到在线流下载器。
- 下载完成后刮削 JavBus 元数据，生成 Jellyfin NFO、封面和预览图。
...
- 真实 `.env`、`cfg/configs.json`、`db/`、`logs/`、视频目录、Cookie、Token、API key、webhook 不应提交。
...
- 屏蔽演员、屏蔽标签、收藏女优、标题关键词是持久化配置，改动前要确认对应接口和文件。

## 前端设计规则

当前视觉方向是粉白色 AV/GARDEN：
...
- 不编造卡片、统计数字、演员、影片数据；没有真实数据就保持空状态。
