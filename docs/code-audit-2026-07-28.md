...
上一轮记录的 9 个延期问题已经全部修复、测试并部署。当时全量媒体复核的 220 个主视频全部通过 `ffprobe`；当前 `/api/videos` 返回 218 部，其余下载恢复属于持续变化的 qB 运行态。
...
| 在线搜索缓存不回收 | 增加 30 天 TTL、启动清理和每小时清理。 |
...
- 未看 Weekly 条目不按年龄删除；手动已看从 `watched_at` 起保留 30 天，屏蔽条目从收录时间起保留 30 天。
- 屏蔽判定改为元数据优先；命中后只保留番号、演员、标签等匹配字段，不请求图片、磁链、标题翻译或下载。
- Launcher 每天 04:30 运行带签名复核、原子写入和 gzip 备份的精确 manifest 清理。在线缓存、应用日志和日常维护记录按 30 天保留。
- 首次生产清单为 `/db/maintenance/manifests/weekly-retention-initial-20260729.json`，结果为同路径追加 `.result.json`。
- 首次清理从 1,811 条 Weekly 中精确删除 844 条过期已看，已看记录从 1,934 条收口为 1,010 条；删除 1,315 个过期或屏蔽图片目录、7,470 个文件，释放约 795.66 MiB 已分配空间。
- 清理后 Weekly 原始数据 967 条、API 可见 496 条、图片目录 496 个，孤儿目录、屏蔽图片目录和超过 30 天的在线缓存/日志文件都为 0。

## 验证结果

- Python：106 项通过，2 项仅容器环境测试在本地跳过；两项 Worker 集成路径已在生产容器单独验证。
- Go：race 测试与 `go vet` 通过。
- 前端：Node 测试和生产构建通过。
- npm：官方 registry 审计为 0 漏洞。
- 媒体：220 个统一主视频全部通过 `ffprobe`，失败 0、小 MP4 0、非多段重复候选 0。
- Weekly：首次保留清理后原始数据 967 条、已看记录 1,010 条；471 条仍在保留期内的屏蔽作品均为无图片的最小元数据，孤儿目录 0。
...
- Docker：两个容器健康，本次部署仅删除 2 个被替换且无容器引用的旧镜像。

## 日常维护

`tools/maintenance/weekly_retention_maintenance.py` 是当前自动保留入口，Launcher 每天 04:30 以 `--auto` 运行。`weekly_cache_maintenance.py` 仍用于独立的 Weekly 孤儿图片复核，默认只把“不在当前 JSON 且超过 30 天”的直接子目录写入 dry-run manifest。
